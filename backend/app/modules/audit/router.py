...
async def list_logs(user: User = Depends(require_web_owner), db: AsyncSession = Depends(get_db)):
...
async def entity_history(entity_type: str, entity_id: UUID, user: User = Depends(require_web_owner), db: AsyncSession = Depends(get_db)):
