...
async def get_account(customer_id: UUID, user: User = Depends(require_company_app_user), db: AsyncSession = Depends(get_db)):
...
async def earn_points(data: EarnPointsRequest, user: User = Depends(require_company_app_user), db: AsyncSession = Depends(get_db)):
...
async def redeem_points(data: RedeemPointsRequest, user: User = Depends(require_company_app_user), db: AsyncSession = Depends(get_db)):
