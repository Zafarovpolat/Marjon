...
async def create_plan(data: PlanCreate, user: User = Depends(require_hq_admin), db: AsyncSession = Depends(get_db)):
    return await SubscriptionService(db).create_plan(data)


@router.post("", response_model=SubscriptionResponse, status_code=status.HTTP_201_CREATED)
async def subscribe(data: SubscriptionCreate, user: User = Depends(require_web_owner), db: AsyncSession = Depends(get_db)):
...
async def current_subscription(user: User = Depends(require_web_owner), db: AsyncSession = Depends(get_db)):
