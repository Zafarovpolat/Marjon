...
async def create_zone(data: ZoneCreate, user: User = Depends(require_company_app_user), db: AsyncSession = Depends(get_db)):
    return await DeliveryService(db).create_zone(user.company_id, data)


@router.get("/zones", response_model=list[ZoneResponse])
async def list_zones(user: User = Depends(require_company_app_user), db: AsyncSession = Depends(get_db)):
...
async def create_courier(data: CourierCreate, user: User = Depends(require_company_app_user), db: AsyncSession = Depends(get_db)):
...
async def list_couriers(user: User = Depends(require_company_app_user), db: AsyncSession = Depends(get_db)):
    return await DeliveryService(db).list_couriers(user.company_id)


@router.patch("/couriers/{courier_id}/location")
async def update_location(courier_id: UUID, data: LocationUpdate, user: User = Depends(require_company_app_user), db: AsyncSession = Depends(get_db)):
...
async def create_delivery_order(data: DeliveryOrderCreate, user: User = Depends(require_company_app_user), db: AsyncSession = Depends(get_db)):
...
async def assign_courier(delivery_id: UUID, data: CourierAssign, user: User = Depends(require_company_app_user), db: AsyncSession = Depends(get_db)):
...
async def update_status(delivery_id: UUID, data: DeliveryStatusUpdate, user: User = Depends(require_company_app_user), db: AsyncSession = Depends(get_db)):
