...
async def create_customer(data: CustomerCreate, user: User = Depends(require_company_app_user), db: AsyncSession = Depends(get_db)):
    return await CustomerService(db).create(user.company_id, data)


@router.get("/customers", response_model=list[CustomerResponse])
async def list_customers(q: str | None = Query(None), user: User = Depends(require_company_app_user), db: AsyncSession = Depends(get_db)):
...
async def get_customer(customer_id: UUID, user: User = Depends(require_company_app_user), db: AsyncSession = Depends(get_db)):
    return await CustomerService(db).get(user.company_id, customer_id)


@router.patch("/customers/{customer_id}", response_model=CustomerResponse)
async def update_customer(customer_id: UUID, data: CustomerUpdate, user: User = Depends(require_company_app_user), db: AsyncSession = Depends(get_db)):
...
async def add_note(customer_id: UUID, data: NoteCreate, user: User = Depends(require_company_app_user), db: AsyncSession = Depends(get_db)):
