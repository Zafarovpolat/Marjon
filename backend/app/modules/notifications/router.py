...
async def send_notification(data: NotificationCreate, user: User = Depends(require_company_app_user), db: AsyncSession = Depends(get_db)):
    return await NotificationService(db).send(user.company_id, data)


@router.get("/unread", response_model=list[NotificationResponse])
async def get_unread(user: User = Depends(require_company_app_user), db: AsyncSession = Depends(get_db)):
    return await NotificationService(db).get_unread(user.company_id, user.id)


@router.patch("/{notification_id}/read", response_model=NotificationResponse)
async def mark_read(notification_id: UUID, user: User = Depends(require_company_app_user), db: AsyncSession = Depends(get_db)):
