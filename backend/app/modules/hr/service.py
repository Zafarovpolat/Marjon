...
class HRService:
    ...
    async def log_attendance(self, company_id: UUID, data: AttendanceCreate) -> AttendanceLog:
        # Tenant-проверка (upstream): чужого employee/shift не подхватываем.
        # employee_id/shift_id у нас опциональны (5.5 — отметка повара без
        # привязки к смене), поэтому связь сверяем только когда оба переданы.
        ...
        if employee is not None and shift is not None and shift.employee_id != employee.id:
