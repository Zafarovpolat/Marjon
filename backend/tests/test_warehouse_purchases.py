...
async def test_accepting_purchase_increases_stock(client):
    ...
    before = await client.get("/inventory/stock", headers=stock_headers)
    assert not any(s["ingredient_id"] == ing["id"] for s in before.json())

    accepted = await client.patch(f"/warehouse/purchases/{doc_id}", headers=stock_headers, json={"status": "accepted"})
...
async def test_accepting_purchase_twice_is_idempotent(client):
    ...
    await client.patch(f"/warehouse/purchases/{doc_id}", headers=stock_headers, json={"status": "accepted"})
    await client.patch(f"/warehouse/purchases/{doc_id}", headers=stock_headers, json={"status": "accepted"})
