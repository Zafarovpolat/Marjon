...
export default function AnalyticsPage() {
  ...
  useEffect(() => {
    ...
    analyticsService.listSales(getDayRange(selectedDate).date_from, getDayRange(selectedDate).date_to, { signal: request.signal })
      ...
      .catch((err) => { if (request.isCurrent() && !isAbortError(err)) { setRows([]); setError(err.response?.data?.detail || "\u041d\u0435 \u0443\u0434\u0430\u043b\u043e\u0441\u044c \u0437\u0430\u0433\u0440\u0443\u0437\u0438\u0442\u044c \u0430\u043d\u0430\u043b\u0438\u0442\u0438\u043a\u0443."); } })
  ...
  return (
    <section className="card card-pad">
      ...
      <div className="table-responsive"><table className="data-table"><thead><tr><th>{"\u0414\u0430\u0442\u0430"}</th><th>{"\u0417\u0430\u043a\u0430\u0437\u044b"}</th><th>{"\u0412\u044b\u0440\u0443\u0447\u043a\u0430"}</th><th>{"\u0421\u0440\u0435\u0434\u043d\u0438\u0439 \u0447\u0435\u043a"}</th></tr></thead><tbody>
        {loading ? <tr><td colSpan="4" role="status">{"\u0417\u0430\u0433\u0440\u0443\u0437\u043a\u0430..."}</td></tr> : rows.map((row) => <tr key={row.date}><td>{row.date}</td><td>{formatNumber(row.orders_count)}</td><td>{formatMoney(row.revenue)}</td><td>{formatMoney(row.avg_check)}</td></tr>)}
        {!loading && !error && !rows.length ? <tr><td colSpan="4">{"\u0414\u0430\u043d\u043d\u044b\u0445 \u043f\u043e \u043f\u0440\u043e\u0434\u0430\u0436\u0430\u043c \u0437\u0430 \u044d\u0442\u043e\u0442 \u0434\u0435\u043d\u044c \u043d\u0435\u0442."}</td></tr> : null}
      </tbody></table></div>
    </section>
  );
}
